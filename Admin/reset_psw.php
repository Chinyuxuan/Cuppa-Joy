...
if (isset($_POST["password"]) && isset($_POST["newcfpass"])) {
    include('../user/db_connection.php');
    $psw = $_POST["password"];
    $pswConfirm = $_POST["newcfpass"];
    ...
    if (isset($_GET['token']) && isset($_GET['email'])) {
        ...
        if ($query > 0) {
            $currentPasswordHash = $fetch['S_Password'];
            if (password_verify($psw, $currentPasswordHash)) {
                echo "<script>alert('The new password cannot be the same as the old password.'); window.history.back();</script>";
                exit;
            }
            $newPassHash = password_hash($psw, PASSWORD_DEFAULT);
            mysqli_query($con, "UPDATE `staff` SET S_Password='$newPassHash' WHERE S_Email='$email'");
                            ...
                            <div class="card mb-3">
                                <div class="card-body">
                                    ...
                                    <form class="row g-3 needs-validation" novalidate method="post" onsubmit="return validateForm()">
                                        ...
                                        <div class="col-12">
                                            <input class="btn btn-primary w-100" type="submit" value="Submit" onclick="submitForm(event)">
                                        </div>
                                    </form>
                                </div>
                            </div>
